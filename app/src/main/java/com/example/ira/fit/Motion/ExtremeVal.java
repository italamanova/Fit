	...
	public static boolean analyzeMIN_MAX(ExtremeVal v, float epsilon) {
		...
		while (ev != null) {
			if (ev.isMinimum() && (Math.abs(ev.getValue()) > epsilon)) {
				...
				while (evNext != null) {
					if (!evNext.isMinimum() && (Math.abs(evNext.getValue()) > epsilon)) {
						ok = true;
						break;
					} else {
...
//System.out.println("2="+evNext);
						...
						while (evNextNext != null) {
							if (evNextNext.isMinimum() && (Math.abs(evNextNext.getValue()) > epsilon)) {
//System.out.println("3="+evNextNext);
	...
	public static boolean analyzeMAX_MIN_MAX(ExtremeVal v, float epsilon) {
		boolean ok = false;
		ExtremeVal ev = v;
		while (ev != null) {
			if (!ev.isMinimum() && (Math.abs(ev.getValue()) > epsilon)) {
